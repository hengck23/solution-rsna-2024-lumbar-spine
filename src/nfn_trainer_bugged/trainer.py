...
def run_trainer(cfg):
	...
	train_dataset = SplineDataset(processed_df, train_id, cfg=cfg, augment=make_train_augment, mode='train')
	...
	def message_header():
		...
		text += f'                            |         loss                 |  loss                 |                  \n'
		text += f'rate      iter       epoch  |  y_acc  zxy_mask  grade  lb  |  zxy  zxy_mask  grade |  time  \n'
		text += f'-------------------------------------------------------------------------------------------------------\n'
				 #1.00e-4  00000388*    1.00  |  0.638  0.672  0.853  0.816  |  7.250  0.676  0.837  |  0 hr 03 min :  44 gb
		text = text[:-1]
		return text
	...
	def do_valid(net, iteration):
		...
		for t, batch in enumerate(valid_loader):
			...
			B = len(batch['index'])
			num_valid += B
			result.zxy_mask_loss += B * output['zxy_mask_loss'].item()
			result.grade_loss += B * output['grade_loss'].item()

			result.D += batch['D']
			result.xy_truth.append(batch['xy'].data.cpu().numpy())
		...
		# ----
		zxy_mask_loss = result.zxy_mask_loss / num_valid
		...
		diff  = np.abs(xy - xy_truth)
		diffy = diff[..., 1]
		y_acc = (diffy<2).mean()

		valid_loss = [
			y_acc,  zxy_mask_loss, grade_loss, weighted_loss
		]
		return valid_loss
	...
	while break_while_loop==0:
		...
		for t, batch in enumerate(train_loader):
			...
			if iteration % iter_valid == 0:
				#valid_loss = do_valid( net, iteration )
			...
			with torch.cuda.amp.autocast(enabled=cfg.is_amp):
				output = net(batch)
				zxy_loss = output['zxy_loss']
				zxy_mask_loss = output['zxy_mask_loss']
				grade_loss = output['grade_loss']

				loss = zxy_loss + zxy_mask_loss + grade_loss
				batch_loss = [ zxy_loss.item(), zxy_mask_loss.item(), grade_loss.item()]# mask_xy_self_loss.item()]
...
if __name__ == '__main__':
	...
	for f in [3]:
		...
		cfg.resume_from.checkpoint = \
			None # cfg.fold_dir + '/checkpoint/00015720.pth'
		run_trainer(cfg)
